...
/**
* A sequence of RoleType instances.
*/
public final class RoleTypeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public RoleTypeSeq (int initialMaximum) {
        super(RoleType.class, initialMaximum);
    }

    public RoleTypeSeq (Collection elements) {
        super(RoleType.class, elements);
    }

    public  RoleType get(int index) { 
        return (RoleType) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, RoleType.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' RoleType objects (beyond the original size of this object)
